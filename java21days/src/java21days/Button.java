...
public class Button extends JButton {
  ...
  public int row;
  public int column;
  // the button's state (-2 money, -1 cleared, 0 uncleared)
  public int state;
  // teh number of adjacent money bags
  public int near;
  ...
  Button(Board board, int row, int column) {
    super("");
    // store the baord that contains this button
    this.board = board;
  ...
  public void clear(boolean clicked) {
    if (clicked) {
      // this square was manually cleared by the user, so tell the grame to charge the player
    ...
    if (near > 0) {
      ...
      Button left = left();
      Button right = right();
      // if any of these buttons is clear and has no money bags nearby, clear it -- using recursion to fan around the board
      if ((above != null) && (above.state == 0)) {
        // clear the button to the above
        above.clear(false);
      }
      if ((below != null) && (below.state == 0)) {
        // clear the button to the below
        below.clear(false);
      }
  ...
  public void revealMoney() {
    ...
    setText("");
    if (!found) {
      // this bag as never been found so tell the frame to award money to the player
      board.app.earnMoney();
  ...
  // these four directional methods inspect four buttons adjacent to this one returning null on the edges of the game board
  public Button above() {
    if (row > 0) {
