...
public class Board extends JPanel {
  ...
  public final void setup() {
    ...
    while (hidden < money_bags) {
      ...
      if (square[row][column].state == -2) {
        // there's already a money bag in that square
        continue;
      }
  ...
  private int getBagCount(int row, int column) {
    // start counting 
    int count = 0;
    // set the lower and upper boundaries of the search area around a square, making sure not to go outside the game board
  ...
  // set the margins of the game baord to 15 pixels
  @Override
